function [p] = Experiment_CC_normal_training(subject,path_experiment_material)

%% Experiment CC: Normal Training
...
% There must be a ready-to-be-read-in conditionfile (seqFile) for each 
% participant containing:
% 1. target_ID
% 2. target_file (i.e., filename)
% 3. fb_position
% 4. ITI_duration (jittered)
% 5. block (task 1: block 1; task 2: block 2; task 3: block 3)
...
WaitSecs(0.001);
p                   = [];
n_images_and_trials = 128; % number of trials per seqFile
...
% (you can manually change this to your desired output device number!)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    function PresentStimuli
        ...
        for nTrial  = 1:p.presentation.tTrial          
            if nTrial == 1
               ...
               TimeEndStim       = GetSecs;
            elseif nTrial == 33
               WaitSecs('UntilTime', TimeEndStim); % wait until last trial 
                                                   % of former part is over
            ...
            [TimeEndStim] = Trial(nTrial, trial_OnsetTime, target_id, ...
                            target_file, fb_pos, ITI, block_id);
        end
    end
    ...
    function [TimeEndStim] = Trial(nTrial, trial_OnsetTime, target_id, target_file, fb_pos, ITI, block_id)
       ...
       % feedback: visual + auditory + face with red circle
       if block_id == 1
          TimeITIOnset        = trial_OnsetTime; % 1. ITI
          TimeStimOnset       = trial_OnsetTime + ITI; % 2. face
          TimeFeedbackOnset   = trial_OnsetTime + ITI + p.duration.target1; 
                                % 3. feedback; end of response window
          ...
          % turn it ON when time is correct
          ...
          while GetSecs() < TimeFeedbackOnset - 0.05
              ...
              if isnan(response) && ~isnan(current_response) && ...
                      (current_RT > TimeStimOn)
          ...
          % e.g., Ari correct; ORDER important: draw AFTER image
      ...
      %% second block
      % visual + auditory feedback (shorter face duration)
      elseif block_id == 2 
       ...
       save(p.path.path_param,'p'); % save it so that we don't lose data
    end   
    ...
    function SetParams
        ...
        p.stim.nImage                   = size(p.stim.files,1); % number of face files
        ...
        info_fb                    = imfinfo(p.stim.feedback{9,:}); % 200x100: e.g., Ari too slow
