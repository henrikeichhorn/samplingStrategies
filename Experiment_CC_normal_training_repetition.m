function [p] = Experiment_2_normal_training_repetition(subject, part, path_experiment_material)

%% Experiment 2: Normal Training (Repetition)
...
WaitSecs(0.001);
p                   = [];
n_images_and_trials = 16; % number of trials per seqFile
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    function [TimeEndStim] = Trial(nTrial, trial_OnsetTime, target_id, target_file, fb_pos, ITI, block_id)
       ...
       % feedback: visual (face with red circle & text) + auditory
       if block_id == 1 % feedback: visual + auditory + face with red circle
          TimeITIOnset        = trial_OnsetTime; % 1. ITI onset
          TimeStimOnset       = trial_OnsetTime + ITI; % 2. face
          TimeFeedbackOnset   = trial_OnsetTime + ITI + p.duration.target1; % 3. feedback
          TimeEndStim         = trial_OnsetTime + ITI + p.duration.target1 + p.duration.feedback1; % 4. end of feedback
          ...
          %% 1. InterTrialInterval (empty screen)
           ...
           %% 3. Response
           % start collecting response while image is presented
           response = nan;
           RT = nan;
           ...
           if ~isnan(response)  % if there was a response
              ...
              correct = evaluateResponse(target_id, response); % so that we can have feedback
           end        
           ...
           % visual
           % draw face image with red circle
           Screen('DrawTexture', p.ptb.w, p.ptb.stim_fb_red_circles(target_id), [], p.ptb.imrect); 
           % draw text feedback (e.g., Ari correct; ORDER important: draw AFTER image)
    ...
    function SetParams
        ...
        p.path.experiment         = [p.path.baselocation 'normal_training_repetition\']; 
        ...
        p.path.edf                    = sprintf('t%02d%d.edf',subject,part);
        p.part                        = sprintf('part%d',part); 
        ...
        S = load([p.path.experiment 'stimulusfiles\Exp_2_normal_training_rep_sub' num2str(subject) '_block' num2str(part) '.mat']); % loads in seqFile
        ...
        p.duration.target1            = 4.5; % duration of face
        p.duration.feedback1          = 3; % feedback with red circles
        ...
        function [pathtofile] = FileMatrix_instruction(path)
             if mod(subject,4) == 1
                filename1 = 'training_repetition_ver1.png';
             elseif mod(subject,4) == 2
                filename1 = 'training_repetition_ver2.png';
             elseif mod(subject,4) == 3
                filename1 = 'training_repetition_ver3.png';
             elseif mod(subject,4) == 0
                filename1 = 'training_repetition_ver4.png';
             end
             filename2 = 'training_repetition_task.png';
    ...
    function ShowInstruction(nInstruct, waitforkeypress)
        ...
        if waitforkeypress
            ...
            elseif nInstruct == 3 % "Training starts." and "Test starts."
                WaitSecs(2.5+rand(1));
